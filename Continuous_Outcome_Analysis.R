...
OutcomeData <- read.csv( ".../Data/Outcome_Synth_Continuous.csv")
...
for (k in 1:K) {
  ...
  Y_obs[treat_Person_bool &is.na(Y1),1] <- predict(reg1k, newdata= as.data.frame(X_Main_treat[treat_Person_bool &is.na(Y1),]))
  Y_obs[!treat_Person_bool &is.na(Y0),1] <- predict(reg1k, newdata= as.data.frame(X_Main_control[!treat_Person_bool &is.na(Y0),]))    
  
  Y1_Sim[Strata_Person==k &!treat_Person_bool,1] <- predict(reg1k, newdata= as.data.frame(X_Main_treat[Strata_Person==k&treat_Person_bool==0,]))
  Y0_Sim[Strata_Person==k &treat_Person_bool,1] <- predict(reg1k, newdata= as.data.frame(X_Main_control[Strata_Person==k&treat_Person_bool==1,]))
...
for (i in 1:(Sims-1)) {
  ...
  for(k in 1:K){
    ...
    if(num_fac_in_k>0){
      Njk <- Nj[fac_in_k]
      Y_phi <- Y_obs[people_in_k,i] - X_Main_obs[people_in_k,]%*%Beta_full_Sim[[k]][,i]
      
      var_phi <- (Sigma2_Sim[i]*Tau2_PhiY_Sim[i])/(Njk*Tau2_PhiY_Sim[i]+Sigma2_Sim[i])
      mean_den_phi <- Njk + (Sigma2_Sim[i]/Tau2_PhiY_Sim[i])
  ...
  for (k in 1:K) {
    ...
    if(num_in_k>0){
      ...
      X_in_k_t <- t(X_in_k)
      Sig_inv <- 1/Sigma2_Sim[i]
      
      vXkXk_inv <- chol2inv(chol(Sig_inv*X_in_k_t%*%X_in_k + v_beta_k_inv))
      
      Xc_in_k <- X_Main_obs[people_in_k,covsX]
      Betac <- Beta_full_Sim[[k]][covsX,i]
      Betak_hat <- vXkXk_inv%*%(X_in_k_t%*%(Sig_inv*(Y_obs[people_in_k,i]- Xc_in_k%*% Betac-PhiY_Person[people_in_k]))
  ...
  Betak_hat <- vXkXk_inv%*%(X_in_k_t%*%(Sig_inv*(Y_obs[,i]- diag(Xc_in_k%*% Betac)-  PhiY_Person))
                            +v_beta_inv%*%m_beta)
  Sigmak_hat <- vXkXk_inv
  ...
  ExpV=0
  for (k in 1:K) {
    ExpV <-ExpV+ X_in_k%*%Beta_full_Sim[[k]][,new]* as.numeric(Strata_Person==k)
  }
  
  Sigma2_SS = sum((Y_obs[,i]-ExpV-PhiY_Person)^2)
  Sigma2_rate <- b_sigmas+1/2*Sigma2_SS
  Sigma2_shape <- a_sigmas+N/2
  
  Sigma2_Sim[new] <- rinvgamma(1, Sigma2_shape, Sigma2_rate)
  ...
  for (k in 1:K) {
    ...
    Y_dens <- dnorm(Y_obs[,i], Y_mean_k, sqrt(Sigma2_Sim[new]), log = T)
  ...
  for (k in 1:K) {
    ...
    if(sum(Strata_Fac==k)>0){
      ...
      comp_Dens <- dnorm(Y_UnObs_D_k, Y_Mean_UnObs_D_k_comp,sqrt(Sigma2_Sim[new]))*
        pnorm(X_D[UnObs_D_k,]%*%Alpha_Sim[[k]][,new] + PhiD_Person[UnObs_D_k])
      
      noncomp_Dens <- dnorm(Y_UnObs_D_k, Y_Mean_UnObs_D_k_nocomp,sqrt(Sigma2_Sim[new]))*
  ...
  for (k in 1:K) {
    ...
    Ymiss_mean <- X_Main_obs[missingYks,]%*%Beta_full_Sim[[k]][,new] +
      PhiY_Person[missingYks]
    
    Y_obs[missingYks,new] <- rnorm(sum(missingYks),Ymiss_mean,sqrt(Sigma2_Sim[new]))
  ...
  if(i %% 500 ==0){print(i)
    print(table(Strata=S_Sim[,i],Fac_Treat= treat_Fac_bin))}
  ...
  if(new %in% samps_use){
    ...
    for (k in 1:K) {
      ...
      muY_est[k] <- Beta_full_Sim[[k]][1,new]
      Coeff_BetaD1_est[,k] <- Beta_full_Sim[[k]][((2+YCN)+1:YCN),new]
      Coeff_BetaD0_est[,k] <- Beta_full_Sim[[k]][1+1:YCN,new]
      Coeff_D1_est[k] <- Delta1_Sim[k,new]
      Coeff_D0_est[k] <- Delta0_Sim[k,new]
      X_k[[k]] <- X[Strata_Person==k,]
      XD_k[[k]]<- X_D[Strata_Person==k,]
    }
    
    
    ### Computing values of estimands as a function of model parameters ###
    ...
    for (k in 1:K) {
      ...
      XD_Alpha<- as.matrix(XD_k[[k]])%*%Coeff_Alpha_est[,k]
      p_ijk <- pnorm(XD_Alpha/sqrt(Tau2_PhiD_est[k]+1))
      muY_weighted_XBeta0_k <- muY_est[k] +mean(X_k[[k]]%*%Coeff_BetaD0_est[,k])
      weighted_XBeta1_k_delta1_k <- mean(p_ijk*(X_k[[k]]%*%Coeff_BetaD1_est[,k]+ Coeff_D1_est[k])) 
      weighted_delta0_k <- mean(p_ijk*(Coeff_D0_est[k]))
    ...
    ATE1_use[thin_ind] <- ATE_k[1]
    ATE1_Y1_use[thin_ind] <- Y1_k[1]
    ATE1_Y0_use[thin_ind] <- Y0_k[1]
    ATE2_use[thin_ind] <- ATE_k[2]
    ATE2_Y1_use[thin_ind] <- Y1_k[2]
    ATE2_Y0_use[thin_ind] <- Y0_k[2]
    ...
    for (k in 1:K) {
      ...
      weighted_XBeta0_kBeta1_k <- mean(p_ijk*X_k[[k]]%*%(Coeff_BetaD0_est[,k]+Coeff_BetaD1_est[,k]))
      weighted_XBeta0_k <- mean(p_ijk*X_k[[k]]%*%(Coeff_BetaD0_est[,k]))
      Y1_D_k[k] <-  muY_est[k] + weighted_XBeta0_kBeta1_k/p_ijk_mean[k] + Coeff_D1_est[k]
      Y0_D_k[k] <-  muY_est[k] + weighted_XBeta0_k/p_ijk_mean[k] + Coeff_D0_est[k]
      CACE_k[k] <- Y1_D_k[k]-Y0_D_k[k]
    }
    
    CACE1_use[thin_ind] <- CACE_k[1]
    CACE1_Y1_use[thin_ind] <- Y1_D_k[1]
    CACE1_Y0_use[thin_ind] <- Y0_D_k[1]
    CACE2_use[thin_ind]  <- CACE_k[2]
    CACE2_Y1_use[thin_ind] <- Y1_D_k[2]
    CACE2_Y0_use[thin_ind] <- Y0_D_k[2]
    ...
    for (k in 1:K) {
      ATE <- ATE+ pi_est[k]*ATE_k[k]
      ATE_Y1 <- ATE_Y1+ pi_est[k]*Y1_k[k]
      ATE_Y0 <- ATE_Y0+ pi_est[k]*Y0_k[k]
      
      CACE_num <-  CACE_num+ pi_est[k]*CACE_k[k]*p_ijk_mean[k]
      CACE_den <- CACE_den+pi_est[k]*p_ijk_mean[k]
      CACE_Y1_num <-  CACE_Y1_num+ pi_est[k]*Y1_D_k[k]*p_ijk_mean[k]
      CACE_Y0_num <-  CACE_Y0_num+ pi_est[k]*Y0_D_k[k]*p_ijk_mean[k]
    } 
    CACE_Y1_use[thin_ind] <- CACE_Y1_num/CACE_den
    ATE_Y1_use[thin_ind] <- ATE_Y1 
    
    CACE_Y0_use[thin_ind] <- CACE_Y0_num/CACE_den
    ATE_Y0_use[thin_ind] <- ATE_Y0 
    
    CACE_use[thin_ind] <- CACE_num/CACE_den
    ATE_use[thin_ind] <- ATE 
    ...
    for (k in 1:K) {
      ...
      PP_Ymean <- PP_X_Main_obsk%*%Beta_full_Sim[[k]][,new] +
        PP_phiY_person[people_in_k]
      
      PPcheckY[people_in_k,thin_ind] <- rnorm(sum(people_in_k),PP_Ymean,sqrt(Sigma2_Sim[new]))
    }
  }
}
...
plot(1:n.imps, Beta_full_use1[1,],type="l")
plot(1:n.imps, Beta_full_use2[1,],type="l")
plot(1:n.imps, Alpha_Sim[[1]][1,samps_use],type="l")
...
Obs_signal_S1_use <- mean(Obs_signal_S1)
Obs_signal_S2_use <- mean(Obs_signal_S2)

Obs_noise_S1_use <- mean(Obs_noise_S1)
Obs_noise_S2_use <- mean(Obs_noise_S2)
